...
#define kColorRed [UIColor colorWithRed:253/255.0 green:120/255.0 blue:99/255.0 alpha:1]
...
- (id)initWithHeaderTitle:(NSString *)headerTitle
    ...
    if(self){
        ...
        self.headerBackgroundColor = kColorRed;
        ...
        self.confirmButtonBackgroundColor = kColorRed;
...
- (IBAction)cancelButtonPressed:(id)sender{
    [self dismissPicker:^{
        if([self.delegate respondsToSelector:@selector(PopPickerViewDidClickCancelButton:)]){
            [self.delegate PopPickerViewDidClickCancelButton:self];
        }
    }];
}
...
- (IBAction)confirmButtonPressed:(id)sender{
    [self dismissPicker:^{
        if(self.allowMultipleSelection && [self.delegate respondsToSelector:@selector(PopPickerView:didConfirmWithItemsAtRows:)]){
            [self.delegate PopPickerView:self didConfirmWithItemsAtRows:[self selectedRows]];
        }
        
        else if(!self.allowMultipleSelection && [self.delegate respondsToSelector:@selector(PopPickerView:didConfirmWithItemAtRow:)]){
            if (self.selectedIndexPaths.count > 0){
                NSInteger row = ((NSIndexPath *)self.selectedIndexPaths[0]).row;
                [self.delegate PopPickerView:self didConfirmWithItemAtRow:row];
            }
        }
    }];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([self.dataSource respondsToSelector:@selector(PopPickerView:titleForRow:)] && [self.dataSource respondsToSelector:@selector(PopPickerView:imageForRow:)]){
        cell.textLabel.text = [self.dataSource PopPickerView:self titleForRow:indexPath.row];
        cell.imageView.image = [self.dataSource PopPickerView:self imageForRow:indexPath.row];
    } else if ([self.dataSource respondsToSelector:@selector(PopPickerView:attributedTitleForRow:)] && [self.dataSource respondsToSelector:@selector(PopPickerView:imageForRow:)]){
        cell.textLabel.attributedText = [self.dataSource PopPickerView:self attributedTitleForRow:indexPath.row];
        cell.imageView.image = [self.dataSource PopPickerView:self imageForRow:indexPath.row];
    } else if ([self.dataSource respondsToSelector:@selector(PopPickerView:attributedTitleForRow:)]) {
        cell.textLabel.attributedText = [self.dataSource PopPickerView:self attributedTitleForRow:indexPath.row];
    } else if([self.dataSource respondsToSelector:@selector(PopPickerView:titleForRow:)]){
        cell.textLabel.text = [self.dataSource PopPickerView:self titleForRow:indexPath.row];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.allowMultipleSelection){
        ...
        if(!self.needFooterView && [self.delegate respondsToSelector:@selector(PopPickerView:didConfirmWithItemAtRow:)]){
            [self dismissPicker:^{
                [self.delegate PopPickerView:self didConfirmWithItemAtRow:indexPath.row];
            }];
        }
    }
